...
class A1:
    def __init__(
        ...
        """Initialize the Biomni-Lite agent.

        Biomni-Lite does not download a local data lake on startup.
        All 77 curated datasets are available on-demand from Zenodo
        (https://zenodo.org/records/18759554). The agent downloads only
        the files it needs for each task.

        Args:
            path: Working directory for downloaded data and outputs
            llm: LLM model name (default: claude-opus-4-5)
            source: Provider — "OpenAI", "AzureOpenAI", "Anthropic", "Ollama", "Gemini", "Bedrock", or "Custom"
            use_tool_retriever: If True, use smart tool retrieval
            timeout_seconds: Code execution timeout in seconds
            base_url: Base URL for custom model serving
            api_key: API key for custom LLM
            commercial_mode: If True, note non-commercial dataset restrictions
        ...
        from biomni.env_desc import ZENODO_BASE_URL, data_lake_dict, library_content_dict
        ...
        # Create working directory structure (no bulk download — files fetched on demand)
        data_lake_dir = os.path.join(path, "biomni_data", "data_lake")
        os.makedirs(data_lake_dir, exist_ok=True)
        print(f"Working directory: {path}")
        print(f"Data lake cache: {data_lake_dir}")
        print(f"Dataset catalog available at: {self.zenodo_base_url}")
...
- Biological Data Lake (on-demand from Zenodo)
The datasets listed below are available for download at: {zenodo_base_url}/<filename>
Local cache directory: {data_lake_path}
{data_lake_intro}
To use a dataset, first check if it is already cached locally, then download if needed:
  import os, urllib.request
  dest = "{data_lake_path}/<filename>"
  if not os.path.exists(dest):
      urllib.request.urlretrieve("{zenodo_base_url}/<filename>", dest)
...
- Note on using R packages and Bash scripts:
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
        ...
        format_dict = {
            ...
            "zenodo_base_url": self.zenodo_base_url,
