...
def download_from_zenodo(filename: str, local_data_lake_path: str, record_id: str = "18759554") -> str:
    """Download a single data lake file from Zenodo on demand.

    All Biomni-Lite data lake files are hosted at:
        https://zenodo.org/records/{record_id}/files/{filename}

    Files are cached locally so subsequent calls return immediately.

    Args:
        filename: The data lake file name (e.g., "gwas_catalog.pkl")
        local_data_lake_path: Local directory to cache the file
        record_id: Zenodo record ID (default: 18759554)

    Returns:
        Full local path to the downloaded file.
    """
    os.makedirs(local_data_lake_path, exist_ok=True)
    local_path = os.path.join(local_data_lake_path, filename)
    ...
    try:
        ...
        with open(local_path, "wb") as f:
            if total_size > 0:
                with tqdm.tqdm(total=total_size, unit="B", unit_scale=True, desc=filename, ncols=80) as pbar:
        ...
        raise RuntimeError(f"Failed to download {filename} from Zenodo: {e}") from e
