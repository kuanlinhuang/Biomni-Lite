...
def download_data_lake_file(filename: str, data_lake_path: str = "./data/biomni_data/data_lake") -> str:
    """Download a Biomni-Lite data lake file from Zenodo on demand.

    Data lake files are hosted at https://zenodo.org/records/18759554.
    Files are cached locally — if already downloaded, this returns immediately.

    Args:
        filename: The data lake file name, e.g. "gwas_catalog.pkl", "DisGeNET.parquet"
        data_lake_path: Local cache directory (default: ./data/biomni_data/data_lake)

    Returns:
        Full local path to the downloaded (or cached) file.

    Example:
        path = download_data_lake_file("gwas_catalog.pkl")
        import pickle
        gwas = pickle.load(open(path, "rb"))
    """
    from biomni.utils import download_from_zenodo
    return download_from_zenodo(filename, data_lake_path)
